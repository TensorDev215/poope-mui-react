...
const TypeChangeTabs = styled(Tabs)(() => ({
    '& .MuiTabs-indicator': {
...
export const CoinChartPanel = () => {
    ...
    const handleTypeChange = useCallback((_:unknown, newValue: number) => {
        setChartType(newValue)
    }, [])

    const handleDateRangeChange = useCallback((_: unknown, newValue: number) => {
                ...
                <Stack direction='row'>
                    <TypeChangeTabs value={chartDateRangeValue} onChange={handleDateRangeChange}>
                        {dateRanges.map((item, index) => (
                            <StyledTab
                                label={<StyledTypo color='text.primary'>{item}</StyledTypo>}
                                key={`${item}__${index}`}
                            />
                        ))}
                    </TypeChangeTabs>
